...
<head>
	...
	<script src="https://api-maps.yandex.ru/2.1/?apikey=7a2e04ce-f605-4303-92cb-16b8dac84aaf&lang=ru_RU" type="text/javascript"></script>
	<?php wp_head() ?>
</head>
...
<body>
		...
		<?php if (isMobile()) { ?>
			<div class="header-wrapper">
				...
				<div class="header-dropdown">
					...
					?>
					<a href="tel:<?= get_field('site_phone', 'options') ?>" class="header-phone"><?= get_field('site_phone', 'options') ?></a>
			...
			?>
				<div class="header-wrapper">
					...
					?>
					<a href="tel:<?= get_field('site_phone', 'options') ?>" class="header-phone"><?= get_field('site_phone', 'options') ?></a>
					<button class="header-button" id="order-call">перезвоните мне</button>
				</div>
			<?php } ?>
	</header>
	...
	<div class="order-call modal">
		...
		<div class="wrapper">
			<div class="block">
				<h3>Заказать</br>звонок</h3>
				<p>Введите номер телефона, на который мы сможем</br>позвонить и все рассказать</p>
			</div>
			<form class="block" id='order-call_form'>
				<input type="text" id="order-call_name" placeholder="Имя">
				<input type="text" id="order-call_telephone" placeholder="Телефон">
